...
def map_media(medias, media_type):
    ...
    pub_rating_min = db.session.query(func.min(Media.public_rating)).where(Media.media_type == media_type).scalar()
    pub_rating_max = db.session.query(func.max(Media.public_rating)).where(Media.media_type == media_type).scalar()
    ...
    for entry in medias:
        ...
        mapped['scaled_public_rating'] = remap_value(entry.public_rating, pub_rating_min, pub_rating_max,
